...
A Work stops being a branch. The manifest lives on its own ref — `refs/codepatrol/manifest/<work-id>` — created by Spec, always present, and the only authority on the Work. A branch exists only while there is code: it materializes when something needs one (a stage that reads or writes code, an explicit worktree request, or an artifact), cut from the base as it stands at that moment, with `createdFromCommit` and `baselineCommit` recorded at the cut. A Work that never produces anything never owns a branch. When the branch opens, the manifest is also projected into the base; that copy is a projection, and disagreement with the ref is reported rather than merged.
...
3. Plan normally reads the manifest through its ref. It materializes the Change branch and worktree only when `--worktree` is requested or `work checkout` is called; a Plan that produces only a handoff leaves nothing behind.
